...
int main() {
    ...
    gpio_set_function(MOTOR_PIN, GPIO_FUNC_PWM); [2]
    
    // 2. Find out which hardware PWM slice controls this pin
    uint slice_num = pwm_gpio_to_slice_num(MOTOR_PIN); [2]

    // 3. Set the frequency to 1 kHz
    // Assuming a 150MHz system clock: 150,000,000 / (6.0 * 25000) = 1000 Hz
    pwm_set_clkdiv(slice_num, 6.0f); [2]
    pwm_set_wrap(slice_num, 24999); [2]

    // 4. Set a fixed duty cycle. 
    // Let's set it to 50% (12500 out of 25000)
    pwm_set_chan_level(slice_num, pwm_gpio_to_channel(MOTOR_PIN), 12500); [3]
    
    // 5. Turn it on
    pwm_set_enabled(slice_num, true); [3]
